...
public class mRAdapterManually extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public mRAdapterManually(List<t_store> storesData) {
        this.storesData = storesData;
    }

    public mRAdapterManually(List<t_store> storesData, boolean isListAD) {
        this.storesData = storesData;
        this.isListAD = isListAD;
    }
...
//        Log.i("AuroraJacImgPath",storesData[position].getLogo());
    ...
    public int getItemViewType(int position) {
        return (position+1) % (Static_var.insertADNum+1) == 0 && position>0 && isListAD ? ITEM_TYPE.ITEM_ADVERTISEMENT.ordinal() : ITEM_TYPE.ITEM_STORE.ordinal();
    }
        ...
        if( !isLoaded ){
            ...
            for(t_store item:items){
                ...
                if( (storesData.size()+1)%(Static_var.insertADNum+1)==0 ) {
                    List<data_model> listAD = Static_var.listADManually;
                    if (listAD != null) {
    ...
    public class timerTask extends CountDownTimer {
        ...
        public void onTick(long l) {
            ...
            storesData.add(items.get(tickTimes++));
            notifyItemInserted(storesData.size() - 1);
